...
public class ApplicationSchoolService {
    ...
    public AdmissionTicketResponse getAdmissionTicket(Long userId, Long applicationSchoolId) {
        ...
        ApplicationJpaEntity application = applicationJpaRepository.findById(applicationSchool.getApplicationId())
            .orElseThrow(() -> new CustomRuntimeException(ErrorCode.APPLICATION_NOT_FOUND));

        AdmissionTicketImageJpaEntity admissionTicketImage = admissionTicketImageJpaRepository.findByApplicationId(application.getId());
