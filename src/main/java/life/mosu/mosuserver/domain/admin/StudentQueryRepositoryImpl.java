...
public class StudentQueryRepositoryImpl implements StudentQueryRepository {
    ...
    public Page<StudentListResponse> searchAllStudents(
        ...
        List<StudentListResponse> content = query.fetch().stream()
            .map(tuple -> mapToResponse(tuple, profile, application))
            .toList();

        return new PageImpl<>(content, pageable, total);
    }

    private JPAQuery<Tuple> baseQuery(QProfileJpaEntity profile, QApplicationJpaEntity application) {
        return queryFactory
            .select(
                profile.id,
                profile.userName,
    ...
    private StudentListResponse mapToResponse(Tuple tuple, QProfileJpaEntity profile, QApplicationJpaEntity application) {
